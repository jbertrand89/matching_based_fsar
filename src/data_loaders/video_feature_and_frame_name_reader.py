...
class VideoFeatureAndFrameNameReader(torch.utils.data.Dataset):
    ...
    def load_video_features(self, video_dir, random_generator):
        ...
        video_names_filename = os.path.join(video_dir, "clip_names.txt")
        with open(video_names_filename, "r+") as reader:
            data = reader.readlines()

        video_name = video_dir.split("/")[-1]
        video_frame_names = []
        for line in data:
            frame_names = ",".join([os.path.join(video_name, frame)
                                    for frame in line.strip().split(",")])
            video_frame_names.append(np.array(frame_names))
        ...
        if feature_count <= self.clip_count:
            while video_features.shape[0] < self.clip_count:
                ...
                video_frame_names.append(video_frame_names[-1])

            video_frame_names = ";".join(np.array(video_frame_names))

            return video_features, [0], video_frame_names

        video_frame_names = np.array(video_frame_names)
        ...
        video_frame_names = ";".join(video_frame_names[temporal_positions])

        return video_features, temporal_positions, video_frame_names
    ...
    def __getitem__(self, index: int):
        ...
        for label_id, class_id in enumerate(batch_classes):
            ...
            for video_dir in class_support_video_names:
                video_features, temporal_positions, video_names = self.load_video_features(
                    video_dir, self.random_generator)
                support_set.append(video_features)
                support_labels.append(label_id)
                support_video_names.append(video_names)
            ...
            for video_dir in class_query_video_names:
                ...
                target_video_names.append(video_names)
